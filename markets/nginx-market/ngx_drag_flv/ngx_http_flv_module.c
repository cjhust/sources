...
#define NGX_HTTP_FLV_READ_MUCH_MORE_TAGS              4
...
typedef struct {
    ...
    ngx_uint_t            read_tags;            /* non-script tags */
...
ngx_http_flv_parse_metadata_tag(ngx_http_flv_file_t *flv, uint32_t tag_data_size)
{
    ...
    */
    ...
    if ((flv->start_offset >= flv->end_offset) && (flv->end != -1)) {
        ngx_log_error(NGX_LOG_ERR, flv->log, 0,
                          "ngx_http_flv_parse_metadata_tag: start_offset = %O >= args_end_offset = %O",
...
ngx_flv_metadata_write_keyframes_array(ngx_http_flv_file_t *flv, ngx_buf_t *buf, ngx_array_t *array, ngx_str_t *name, off_t adjustment)
{
    ...
    node = array->elts;
    start_offset = node[flv->start_frame_index];
    for (i = flv->start_frame_index; i <= flv->end_frame_index; i++) {

        if (i == flv->start_frame_index && flv->video_padding) {

            if (name->len == 13) {
                /* filepositions */

                ngx_flv_metadata_write_double(buf, adjustment);
                adjustment += flv->video_tag_size + flv->audio_tag_size;
            } else {
                /* times */

                ngx_flv_metadata_write_double(buf, node[i] + adjustment - start_offset);
                flv->start_timestamp = node[i] + adjustment - start_offset;
            }
        }
...
{
    ngx_uint_t          size, compute_header_length;
    ...
    if (flv->video_format == NGX_HTTP_FLV_VIDEO_AVC_TYPE
        && flv->start_frame_index != 0) {
        flv->video_padding = 1;
    }

    if (flv->video_padding) {
        compute_header_length += (flv->end_frame_index - flv->start_frame_index + 1 + 1) * 9 * 2
    ...
    ngx_flv_metadata_write_keyframes_array(flv, &buf, flv->metadata_filepositions, &filepositions_name, compute_header_length);
...
ngx_http_flv_buf_read_tag(ngx_http_flv_file_t *flv)
{
    ...
    while (flv->buffer_pos < flv->buffer_end) {
        ...
        if (flv->read_tags >= NGX_HTTP_FLV_READ_MUCH_MORE_TAGS) {
            if (flv->metadata_parsed) {
                ngx_log_error(NGX_LOG_WARN, flv->log, 0,
                          "ngx_http_flv_buf_read_tag: metadata parsed, but video_parsed = %ui, audio_parsed = %ui, ignore the abnormal", flv->video_parsed, flv->audio_parsed);
...
ngx_http_flv_process_body(ngx_http_request_t *r)
{
    ...
    tag->memory = 1;
    tag->pos = flv.buffer_start;
    tag->last = flv.buffer_start + sizeof(ngx_flv_header) - 1;
    ...
    *prev = &flv.header_tag;
    ...
    if (!r->main->customize.flv_drag_mode) {
        ...
        if (flv.audio_parsed == 1) {
            ...
            *prev = &flv.audio_tag;
            ...
            ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                          "ngx_http_flv_process_body: video format AVC, but no audio, ignore the abnormal");
    ...
    *prev = &flv.tailer_tag;
    ...
    if (end != -1) {
        if (flv.start >= flv.end) {
            ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                      "ngx_http_flv_process_body: flv.start %O >= flv.end %O", flv.start, flv.end);
