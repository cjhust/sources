...
typedef struct {
    ngx_str_t             metadata;
} ngx_http_mp4_body_filter_ctx_t;
...
static ngx_int_t ngx_http_mp4_forecast_stts_atom(ngx_http_mp4_file_t *mp4, ngx_http_mp4_trak_t *trak);
static ngx_int_t ngx_http_mp4_forecast_stts_data(ngx_http_mp4_file_t *mp4, ngx_http_mp4_trak_t *trak, ngx_uint_t start);
static ngx_int_t ngx_http_mp4_forecast_stss_data(ngx_http_mp4_file_t *mp4, ngx_http_mp4_trak_t *trak);
static ngx_int_t ngx_http_mp4_adjust_start_sec(ngx_http_mp4_file_t *mp4, ngx_http_mp4_trak_t *trak);
static void ngx_http_mp4_adjust_mvhd_duration(ngx_http_mp4_file_t *mp4);


static ngx_int_t ngx_http_mp4_buf_read_moov_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size);
static ngx_int_t ngx_http_mp4_buf_read_mdat_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size);
static size_t ngx_http_mp4_buf_update_mdat_atom(ngx_http_mp4_file_t *mp4, off_t start_offset,
    off_t end_offset);
static ngx_int_t ngx_http_mp4_buf_read(ngx_http_mp4_file_t *mp4, size_t size);
static ngx_int_t ngx_http_mp4_buf_read_atom(ngx_http_mp4_file_t *mp4,
    ngx_http_mp4_atom_handler_t *atom, uint64_t atom_data_size);
static ngx_buf_t * ngx_http_mp4_read_body(ngx_http_request_t *r);
static void ngx_http_mp4_process_body(ngx_http_request_t *r);
static ngx_int_t ngx_http_mp4_compute_offset_handler(ngx_http_request_t *r);
static char * ngx_http_mp4_compute_offset(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
static ngx_int_t ngx_http_mp4_body_filter(ngx_http_request_t *r,
    ngx_chain_t *in);
...
static ngx_int_t ngx_http_mp4_init(ngx_conf_t *cf);
...
static ngx_command_t  ngx_http_mp4_commands[] = {
    ...
    { ngx_string("mp4_compute_offset"),
      NGX_HTTP_LOC_CONF|NGX_CONF_NOARGS,
      ngx_http_mp4_compute_offset,
      0,
      0,
      NULL },
    ...
    { ngx_string("mp4_metadata_size"),
      NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_mp4_conf_t, metadata_size),
      NULL },
...
static ngx_http_module_t  ngx_http_mp4_body_filter_module_ctx = {
    NULL,                          /* preconfiguration */
    ngx_http_mp4_init,             /* postconfiguration */
    NULL,                          /* create main configuration */
    NULL,                          /* init main configuration */
...
ngx_module_t  ngx_http_mp4_body_filter_module = {
    NGX_MODULE_V1,
    &ngx_http_mp4_body_filter_module_ctx,   /* module context */
    ngx_http_mp4_commands,                  /* module directives */
    NGX_HTTP_MODULE,                        /* module type */
    NULL,                                   /* init master */
    NULL,                                   /* init module */
...
static ngx_http_mp4_atom_handler_t  ngx_http_mp4_buf_atoms[] = {
    { "ftyp", ngx_http_mp4_read_ftyp_atom },
    { "moov", ngx_http_mp4_buf_read_moov_atom },
    { "mdat", ngx_http_mp4_buf_read_mdat_atom },
    { NULL, NULL }
};
...
static void
ngx_http_mp4_adjust_mvhd_duration(ngx_http_mp4_file_t *mp4)
{
    u_char                 *atom_header;
    ...
    atom = mp4->mvhd_atom.buf;
    atom_header = atom->pos;
    mvhd_atom = (ngx_mp4_mvhd_atom_t *) atom_header;
    mvhd64_atom = (ngx_mp4_mvhd64_atom_t *) atom_header;
    ...
    if (mvhd_atom->version[0] == 0) {
        ngx_mp4_set_32value(mvhd_atom->duration, length_time);
    } else {
        ngx_mp4_set_64value(mvhd64_atom->duration, length_time);
    }
}
...
ngx_http_mp4_read_stsd_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    if ((ngx_memcmp(stsd_atom->media_name, "hev1", 4) == 0)
        || (ngx_memcmp(stsd_atom->media_name, "hvc1", 4) == 0)) {
        mp4->h265 = 1;
    }
...
static ngx_int_t
ngx_http_mp4_forecast_stts_atom(ngx_http_mp4_file_t *mp4,
    ngx_http_mp4_trak_t *trak)
{
    ngx_buf_t   *data;
    ...
    if (ngx_http_mp4_forecast_stts_data(mp4, trak, 1) != NGX_OK) {
        return NGX_ERROR;
    }

    if (ngx_http_mp4_forecast_stts_data(mp4, trak, 0) != NGX_OK) {
        return NGX_ERROR;
    }
...
static ngx_int_t
ngx_http_mp4_forecast_stts_data(ngx_http_mp4_file_t *mp4,
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
...
static ngx_int_t
ngx_http_mp4_adjust_start_sec(ngx_http_mp4_file_t *mp4,
    ngx_http_mp4_trak_t *trak)
{
    uint32_t               count, duration;
    ngx_buf_t             *data;
    ngx_uint_t             start_sample, start_sec, length;
    ngx_mp4_stts_entry_t  *entry, *end;
    ...
    while (entry < end) {
        ...
        if (start_sample < count) {
            start_sec += start_sample * duration;

            length += (count - start_sample) * duration;
            entry++;

            goto found;
        }

        start_sec += count * duration;
        start_sample -= count;
        entry++;
    }
...
found:
    mp4->start = start_sec * 1000 / trak->timescale;
    ...
    length = length * 1000 / trak->timescale;

    if (mp4->length > length) {
        mp4->length = length;

        ngx_http_mp4_adjust_mvhd_duration(mp4);
    }
...
static ngx_int_t
ngx_http_mp4_forecast_stss_data(ngx_http_mp4_file_t *mp4,
    ngx_http_mp4_trak_t *trak)
{
...
static ngx_int_t
ngx_http_mp4_buf_read_moov_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ngx_int_t             rc;
    ngx_uint_t            no_mdat;
    ngx_buf_t            *atom;
    ...
    if (!no_mdat) {
        ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                          "only supprot the mode that moov before mdat");
        return NGX_ERROR;
    }

    if (ngx_http_mp4_buf_read(mp4, (size_t) atom_data_size) != NGX_OK) {
        return NGX_ERROR;
    }
    ...
    rc = ngx_http_mp4_buf_read_atom(mp4, ngx_http_mp4_moov_atoms, atom_data_size);
...
static ngx_int_t
ngx_http_mp4_buf_read_mdat_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ngx_buf_t  *data;
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 mdat atom");

    if (mp4->moov_atom.buf == NULL) {
        ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                      "ngx_http_mp4_buf_read_mdat_atom: only supprot the mode that moov before mdat");
...
static size_t
ngx_http_mp4_buf_update_mdat_atom(ngx_http_mp4_file_t *mp4, off_t start_offset,
    off_t end_offset)
{
    off_t       atom_data_size;
    u_char     *atom_header;
    ...
    mp4->content_length += atom_header_size;
    ...
    atom->last = atom_header + atom_header_size;
    atom->last_buf = 1;
    atom->last_in_chain = 1;

    mp4->mdat_atom.next = NULL;
...
static ngx_int_t
ngx_http_mp4_buf_read(ngx_http_mp4_file_t *mp4, size_t size)
{
    if (mp4->buffer_pos + size <= mp4->buffer_end) {
        return NGX_OK;
    }
    ...
    ngx_http_mp4_atom_handler_t *atom, uint64_t atom_data_size)
{
    ...
    while (mp4->offset < end) {
        ...
        if (ngx_http_mp4_buf_read(mp4, sizeof(uint32_t)) != NGX_OK) {
            return NGX_ERROR;
        }
        ...
        if (atom_size < sizeof(ngx_mp4_atom_header_t)) {
            ...
            if (atom_size == 1) {
                ...
                if (ngx_http_mp4_buf_read(mp4, sizeof(ngx_mp4_atom_header64_t))
                    != NGX_OK)
                {
                    return NGX_ERROR;
                }
                ...
                ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                              "mp4 atom is too small:%uL",
                              atom_size);
                return NGX_ERROR;
            }
        }

        if (ngx_http_mp4_buf_read(mp4, sizeof(ngx_mp4_atom_header_t)) != NGX_OK) {
            return NGX_ERROR;
        }
...
static ngx_buf_t *
ngx_http_mp4_read_body(ngx_http_request_t *r)
{
    size_t                len;
    ssize_t               size;
    ngx_buf_t            *buf, *body;
...
static void
ngx_http_mp4_process_body(ngx_http_request_t *r)
{
    ngx_int_t                       rc, start, end;
    ngx_uint_t                      i, j, length;
    ngx_str_t                       value, tmp_value;
    off_t                           start_offset, end_offset, adjustment;
    ngx_table_elt_t                *mp4_video_format;
    ngx_buf_t                      *buf;
    ngx_chain_t                   **prev;
    ngx_http_mp4_trak_t            *trak;
    ngx_mp4_mvhd_atom_t            *mvhd_atom;
    ...
    tmp_value.data = ngx_pcalloc(r->pool, r->args.len + 1);
    if (tmp_value.data == NULL) {
    ...
    if (r->args.len) {
        ...
        if (ngx_http_arg(r, (u_char *) "start", 5, &value) == NGX_OK) {
            ...
            ngx_memcpy(tmp_value.data, value.data, value.len);
            tmp_value.len = value.len;
            tmp_value.data[tmp_value.len] = '\0';

            start = (int) (strtod((char *) tmp_value.data, NULL) * 1000);
        ...
        if (ngx_http_arg(r, (u_char *) "end", 3, &value) == NGX_OK) {
            ...
            end = (int) (strtod((char *) tmp_value.data, NULL) * 1000);
    ...
    buf = ngx_http_mp4_read_body(r);
    if (buf == NULL) {
          ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "ngx_http_mp4_process_body: body is NULL or too large");
    ...
    rc = ngx_http_mp4_buf_read_atom(&mp4, ngx_http_mp4_buf_atoms, mp4.end);
    if (rc != NGX_OK) {
        ngx_log_error(NGX_LOG_WARN, mp4.file.log, 0,
    ...
    if (mp4.trak.nelts == 0) {
        ngx_log_error(NGX_LOG_ERR, mp4.file.log, 0,
                      "ngx_http_mp4_process_body: no mp4 trak atoms were found");
    ...
    *prev = &mp4.moov_atom;
    ...
    for (i = 0; i < mp4.trak.nelts; i++) {
        /* forecast start */
        if (ngx_http_mp4_forecast_stts_atom(&mp4, &trak[i]) != NGX_OK) {
            ngx_http_finalize_request(r, NGX_HTTP_BAD_REQUEST);
            return;
        }

        if (ngx_http_mp4_forecast_stss_data(&mp4, &trak[i]) != NGX_OK) {
            ngx_http_finalize_request(r, NGX_HTTP_BAD_REQUEST);
            return;
        }

        if (ngx_http_mp4_adjust_start_sec(&mp4, &trak[i]) != NGX_OK) {
            ngx_http_finalize_request(r, NGX_HTTP_BAD_REQUEST);
            return;
        }
        /* forecast end */
    ...
    *prev = &mp4.mdat_atom;
    ...
    adjustment = mp4.ftyp_size + mp4.moov_size
                 + ngx_http_mp4_buf_update_mdat_atom(&mp4, start_offset, end_offset)
                 - start_offset;
    ...
    r->headers_out.content_type_len = ngx_sprintf(r->headers_out.content_type.data, "%O-%O", start_offset, end_offset - 1) - r->headers_out.content_type.data;
    r->headers_out.content_type.len = r->headers_out.content_type_len;

    r->headers_out.status = NGX_HTTP_OK;
    r->headers_out.content_length_n = mp4.content_length;

    if (mp4.mvhd_atom.buf != NULL && r->customize.mp4_limit_rate_factor > 0) {
        r->customize.mp4_timescale = mp4.timescale;

        mvhd_atom = (ngx_mp4_mvhd_atom_t *) mp4.mvhd_atom_buf.pos;
        mvhd64_atom = (ngx_mp4_mvhd64_atom_t *) mp4.mvhd_atom_buf.pos;

        if (mvhd_atom->version[0] == 0) {
            r->customize.mp4_duration = ngx_mp4_get_32value(mvhd_atom->duration);

        } else {
            r->customize.mp4_duration = ngx_mp4_get_64value(mvhd64_atom->duration);
        }
    }

    /* mp4-video-format */
    if (mp4.h265) {
        mp4_video_format = ngx_list_push(&r->headers_out.headers);
        if (mp4_video_format == NULL) {
            ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }

        mp4_video_format->hash = 1;

        mp4_video_format->key.data = ngx_pnalloc(r->pool, 64);
        mp4_video_format->value.data = ngx_pnalloc(r->pool, 64);
        if ((mp4_video_format->key.data == NULL)
            || (mp4_video_format->value.data == NULL)) {
            ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }

        mp4_video_format->key.len = ngx_sprintf(mp4_video_format->key.data, "mp4-video-format") - mp4_video_format->key.data;
        mp4_video_format->value.len = ngx_sprintf(mp4_video_format->value.data, "H265") - mp4_video_format->value.data;
    }
    ...
    ngx_http_finalize_request(r, ngx_http_output_filter(r, mp4.out));
}
...
static ngx_int_t
ngx_http_mp4_compute_offset_handler(ngx_http_request_t *r)
{
    ngx_int_t    rc;

    if (r->method != NGX_HTTP_POST) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "ngx_http_mp4_compute_offset_handler: need POST method");

        return NGX_HTTP_BAD_REQUEST;
    }

    rc = ngx_http_read_client_request_body(r, ngx_http_mp4_process_body);
...
static ngx_int_t
ngx_http_mp4_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ssize_t                         size;
    size_t                          metadata_size;
    double                          byterate;
    uint32_t                        timescale;
    uint64_t                        duration;
    ngx_int_t                       rc, limit_rate_factor;
    ngx_msec_int_t                  ms, limit_rate_after;
    ngx_http_mp4_file_t             mp4;
    ngx_chain_t                    *cl;
    ngx_time_t                     *tp;
    ngx_http_mp4_conf_t            *conf;
    ngx_mp4_mvhd_atom_t            *mvhd_atom;
    ngx_mp4_mvhd64_atom_t          *mvhd64_atom;
    ngx_http_mp4_body_filter_ctx_t *ctx;

    ctx = ngx_http_get_module_ctx(r, ngx_http_mp4_body_filter_module);
    conf = ngx_http_get_module_loc_conf(r, ngx_http_mp4_module);
    metadata_size = conf->metadata_size;

    limit_rate_after = r->customize.mp4_limit_rate_after;
    limit_rate_factor = r->customize.mp4_limit_rate_factor;

    if (limit_rate_factor == 0
        || r->limit_rate
        || r->headers_out.status != NGX_HTTP_OK
        || r->headers_out.content_length_n < (off_t) metadata_size)
    {
        goto next_filter;
    }

    if (ctx == NULL) {
        ctx = ngx_pcalloc(r->pool, sizeof(ngx_http_mp4_body_filter_ctx_t));
        if (ctx == NULL) {
            return NGX_ERROR;
        }

        ngx_http_set_ctx(r, ctx, ngx_http_mp4_body_filter_module);
    }
    ...
    /* duration and timescale have been calculated when drag happens */
    if (r->customize.mp4_duration > 0) {
        if (ms < limit_rate_after) {
            goto next_filter;
        }

        timescale = r->customize.mp4_timescale;
        duration = r->customize.mp4_duration;

        goto limit_rate;
    }

    /* FIXME some memory consumption here */

    if (ctx->metadata.data == NULL) {
        ctx->metadata.data = ngx_palloc(r->pool, metadata_size);
        if (ctx->metadata.data == NULL) {
            return NGX_ERROR;
        }
    }
    ...
    /*
     * cached the first metadata_size bytes,
     * for decoding the moov atom.
     */

    for (cl = in; cl; cl = cl->next) {
        if (ctx->metadata.len >= metadata_size) {
            break;
        }
        ...
        if (size + ctx->metadata.len >= metadata_size) {
            size = metadata_size - ctx->metadata.len;
        }

        if (!cl->buf->in_file) {
            ngx_memcpy(ctx->metadata.data + ctx->metadata.len,
                       cl->buf->pos, size);

        } else {
            size = ngx_read_file(cl->buf->file,
                                 ctx->metadata.data + ctx->metadata.len,
                                 size,
                                 cl->buf->file_pos);

            if (size == NGX_ERROR) {
                ngx_pfree(r->pool, ctx->metadata.data);
                ngx_str_null(&ctx->metadata);
                return NGX_ERROR;
            }
        }

        ctx->metadata.len += size;
    }
    ...
    ngx_memzero(&mp4, sizeof(ngx_http_mp4_file_t));
    mp4.file.log = r->connection->log;
    mp4.file.name.data = (u_char *) "mp4_limit_rate";
    mp4.file.name.len = sizeof("mp4_limit_rate") - 1;
    mp4.buffer_start = ctx->metadata.data;
    mp4.buffer_pos = ctx->metadata.data;
    mp4.buffer_end = ctx->metadata.data + ctx->metadata.len;
    mp4.length = 0;
    mp4.end = ctx->metadata.len;
    mp4.request = r;

    rc = ngx_http_mp4_buf_read_atom(&mp4, ngx_http_mp4_buf_atoms, mp4.end);
    if (rc != NGX_OK || mp4.mvhd_atom.buf == NULL) {
        if (rc == NGX_HTTP_NOT_IMPLEMENTED) {
            ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                          "mp4 metadata size too large");

        } else {
            ngx_log_error(NGX_LOG_WARN,  r->connection->log, 0,
                          "mp4 metadata parsed failed");
        }
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "ngx_http_mp4_body_filter(): timescale = %ud, duration=%ul",
                    timescale, duration);
...
limit_rate:
    ...
    byterate = r->headers_out.content_length_n * 1.0 / duration * timescale;

    r->limit_rate = (size_t) (limit_rate_factor / 1000.0 * byterate);

    r->limit_start_sec = r->start_sec + limit_rate_after / 1000;

    ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                  "ngx_http_mp4_body_filter(): mp4 limit rate triggered, "
                  "limit_rate: %uz", r->limit_rate);
...
static char *
ngx_http_mp4_compute_offset(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_core_loc_conf_t  *clcf;

    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);
    clcf->handler = ngx_http_mp4_compute_offset_handler;
...
ngx_http_mp4_create_conf(ngx_conf_t *cf)
{
    ...
    conf->metadata_size = NGX_CONF_UNSET_SIZE;
...
ngx_http_mp4_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_size_value(conf->max_buffer_size, prev->max_buffer_size,
                              10 * 1024 * 1024);
    ngx_conf_merge_size_value(conf->metadata_size, prev->metadata_size,
                              512 * 1024);
...
static ngx_int_t
ngx_http_mp4_init(ngx_conf_t *cf)
{
    ngx_http_next_body_filter = ngx_http_top_body_filter;
    ngx_http_top_body_filter = ngx_http_mp4_body_filter;
